...
if [ -z "$SCHEMA_FILE" ]; then
    ...
    log "downloading schema files ..."
    wget $SCHEMA_FILE
    mv schema.up.sql "$HASURA_GRAPHQL_MIGRATIONS_DIR"/1$(tr -cd 0-9 </dev/urandom | head -c 16)_schema.up.sql
fi

if [ -z "$METADATA_FILE" ]; then
    log "can't find metadata file to download ..."
else
    log "downloading metadata files ..."
    wget $METADATA_FILE
    mv metadata.up.yaml "$HASURA_GRAPHQL_MIGRATIONS_DIR"/2$(tr -cd 0-9 </dev/urandom | head -c 16)_metadata.up.yaml
fi
...
if [ -d "$HASURA_GRAPHQL_MIGRATIONS_DIR" ]; then
    ...
    # run migrations and output result
    echo $(hasura-cli migrate apply)
